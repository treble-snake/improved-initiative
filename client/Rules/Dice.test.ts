...
describe("DiceRoll", () => {
  ...
  test("calculates the maximum", () => {
    expect(createRoll([3, 6, 7], 4, 12).Maximum).toBe(40);
  });
  ...
  test.each([RollModes.Advantage, RollModes.Disadvantage])(
    "calculates the maximum for %s using one die",
    mode => {
      expect(createRoll([3, 14], 4, 20, mode).Maximum).toBe(24);
    }
  );
